...
//
// LRTF is PREEMPTIVE — the running process can be interrupted
// every single time unit if a process with a longer remaining
// time shows up. A single process can appear in MULTIPLE Gantt
// segments across the timeline. We merge consecutive 1-unit
// executions of the SAME process into a single segment (see
// appendGanttSegment below) so the chart doesn't render one
// tiny box per time unit.
// ============================================================
struct GanttSegment{
...
//
// Since LRTF executes 1 time unit at a time, naively recording
...
// Max Heap stores {Remaining Time, -Arrival Time, Index}   // minus sign (-) is used before Arrival Time to make it a Max Heap, as priority_queue in C++ is by default a Max Heap.
// If the Remaining Time is same for two processes, then the process with the earlier Arrival Time will be given priority. Thus as it is a Max Heap, we use -Arrival Time to give priority to the process with the earlier Arrival Time. (arrival time : 4>2  ;  but we want to choose the process with arrival time 2, thus we use -Arrival Time to make it -4<-2, thus process with arrival time 2 will be given priority)
// Algorithm:
// 1. Sort processes by Arrival Time.
// 2. Push all arrived processes into the Max Heap.
// 3. Pick the process with the longest Remaining Time.
...
// we're still picking longest-remaining-time exactly as before.
// ============================================================
void findCompletionTime(vector<Process>& processes, vector<GanttSegment>& ganttChart){
    ...
    // {Remaining Time, -Arrival Time, Index}
    ...
    while(completed < n){
        ...
        while(i<n && processes[i].arrivalTime<=currentTime){
            ...
            pq.push({{processes[i].remainingTime, 
...
// (much smaller than total time units, thanks to merging).
// ============================================================
...
// ============================================================
// runLRTFScheduling is the ONE function FastAPI will call (via
...
// ============================================================
SimulationResult runLRTFScheduling(const vector<ProcessInput>& processInputs){
...
int main(){
    ...
    SimulationResult result = runLRTFScheduling(processInputs);
