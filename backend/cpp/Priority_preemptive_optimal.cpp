...
//
// Priority Preemptive can interrupt the running process every
// single time unit if a higher-priority process arrives. A
// single process can appear in MULTIPLE Gantt segments across
// the timeline. We merge consecutive 1-unit executions of the
// SAME process into a single segment (see appendGanttSegment
// below) so the chart doesn't render one tiny box per time unit.
// ============================================================
struct GanttSegment{
...
//
// Since this algorithm executes 1 time unit at a time, naively
// recording a Gantt segment per unit would produce hundreds of
// 1-unit-wide segments for a process that actually ran
// uninterrupted for, say, 5 units. Instead, we check: does the
// last segment belong to the SAME process and does it end
// exactly where this new unit starts? If so, we just extend it.
// Otherwise, we start a brand new segment (a real context switch
// or idle gap happened).
//
...
// we're still picking highest-priority exactly as before.
// ============================================================
void findCompletionTime(vector<Process>& processes, vector<GanttSegment>& ganttChart){
    ...
    while(completed<n){
        ...
        // Push back if unfinished
            ...
            pq.push({{processes[idx].priority,
...
// ============================================================
// runPriorityPreemptiveScheduling is the ONE function FastAPI
// will call (via the compiled executable) for this algorithm.
//
...
// ============================================================
SimulationResult runPriorityPreemptiveScheduling(const vector<ProcessInput>& processInputs){
